...
function isTshirtCategory(cate: string) {
  const c = cate.toLowerCase();
  return c.includes('เสื้อยืด') || c.includes('t-shirt') || c.includes('tshirt') || c.includes('tee');
}
...
export default function TshirtPage() {
  ...
  const filtered = useMemo(
    () => items.filter((p) => isTshirtCategory(pickCategoryValue(p))),
    [items]
  );
    ...
    <>
      ...
      <div className={styles.page}>
        <div className={styles.container}>
          <h1 className={styles.title}>เสื้อยืด</h1>
